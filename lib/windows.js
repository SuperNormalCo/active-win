...
function reinterpretUntilZeros(buffer) {
	// See sindresorhus/active-win#65
	//return ref.reinterpretUntilZeros(buffer, wchar.size);	
	const clean = wchar.toString(buffer).replace(/\0+/, '');
	return Buffer.from(clean, 'ucs2');
}
...
function windows() {
	...
	const windowTextBufferClean = reinterpretUntilZeros(windowTextBuffer);
	...
	const processFileNameBufferClean = reinterpretUntilZeros(processFileNameBuffer);
